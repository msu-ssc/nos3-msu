...
** Include Files
*/
#include "sample_radio_checkout.h"
...
void print_help(void)
{
    printf(PROMPT "command [args]\n"
                  ...
                  "  h                                - ^                               \n"
                  "sample_radio                             - Request sample_radio data             \n"
                  "  s                                - ^                               \n"
...
int get_command(const char *str)
{
    ...
    else if (strcmp(lcmd, "sample_radio") == 0)
    {
        status = CMD_SAMPLE_RADIO;
    }
    ...
    switch (cc)
    {
        ...
        case CMD_HK:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                ...
                if (status == OS_SUCCESS)
                {
                    OS_printf("SAMPLE_RADIO_RequestHK command success\n");
                }
                else
                {
                    OS_printf("SAMPLE_RADIO_RequestHK command failed!\n");
                }
            }
            break;

        case CMD_SAMPLE_RADIO:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = SAMPLE_RADIO_RequestData(&Sample_radioUart, &Sample_radioData);
                if (status == OS_SUCCESS)
                {
                    OS_printf("SAMPLE_RADIO_RequestData command success\n");
                }
                else
                {
                    OS_printf("SAMPLE_RADIO_RequestData command failed!\n");
                }
            }
            break;
...
/* Initialize HWLIB */
    ...
    // Close the device
    uart_close_port(&Sample_radioUart);

#ifdef _NOS_ENGINE_LINK_
    nos_destroy_link();
#endif

    OS_printf("Cleanly exiting sample_radio application...\n\n");
    return 1;
}
