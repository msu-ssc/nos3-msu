# Sample_radio - NOS3 Component
This repository contains the NOS3 Sample_radio Component.
This includes flight software (FSW), ground software (GSW), simulation, and support directories.

## Component Template
This component template utilizes canned files from the existing [sample_radio component](https://github.com/nasa-itc/sample_radio) to generate a new component to do development in.
...
## Overview
In addition to being used by the template generator, the sample_radio component provides an executable example for the user.
This sample_radio component is a UART device that accepts multiple commands, including requests for telemetry and data.
The available FSW is for use in the core Flight System (cFS) while the GSW supports COSMOS.
A NOS3 simulation is available which includes both sample_radio and 42 data providers.
...
* Sample_radio
  ...
  - uint32, Command Counter
    * Increments for each command received
  - uint16, Data X
    * X component of sample_radio data
  - uint16, Data Y
    * X component of sample_radio data
  - uint16, Data Z
    * X component of sample_radio data
...
## FSW
Refer to the file [fsw/platform_inc/sample_radio_platform_cfg.h](fsw/platform_inc/sample_radio_platform_cfg.h) for the default
...
```
<simulator>
    <name>sample_radio_sim</name>
    <active>true</active>
    <library>libsample_radio_sim.so</library>
    <hardware-model>
        <type>SAMPLE_RADIO</type>
        <connections>
            <connection><type>command</type>
                <bus-name>command</bus-name>
                <node-name>sample_radio-sim-command-node</node-name>
            </connection>
        ...
        <data-provider>
            <type>SAMPLE_RADIO_PROVIDER</type>
        </data-provider>
    </hardware-model>
</simulator>
```
...
```
        <data-provider>
            <type>SAMPLE_RADIO_42_PROVIDER</type>
...
* make debug
* cd ./components/sample_radio/support
* mkdir build
* cd build
* cmake .. 
...
* make
* make checkout
  * Launches NOS Engine, NOS Time Driver, NOS Terminal, Sample_radio Sim, and Sample_radio Checkout
* make stop

# Documentation
If this sample_radio application had an ICD and/or test procedure, they would be linked here.
